...
class HcFcCnn(nn.Module):
    def __init__(self, num_classes):
        ...
        self.hc_conv = nn.Conv2d(64 + 128 + 256 + 512, 512, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        hc_inputs.append(F.upsample_bilinear(x, scale_factor=2))
        x = self.max_pool2(x)

        x = self.conv3(x)
        hc_inputs.append(F.upsample_bilinear(x, scale_factor=4))
        x = self.max_pool3(x)

        x = self.conv4(x)
        hc_inputs.append(F.upsample_bilinear(x, scale_factor=8))
